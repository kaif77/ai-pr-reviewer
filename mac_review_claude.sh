...
# ─────────────────────────────────────────
# AI PR Reviewer - Mac compatible + caching
# ─────────────────────────────────────────
...
if [ -f "$ENV_FILE" ]; then
 echo -e "${GREEN}✅ Loading .env file from $ENV_FILE${NC}"
 export $(tr -d '\r' < "$ENV_FILE" | xargs)
else
...
# ── Step 8: Check cache ──
# Cache key = diff filename + repo + PR number
# Safe filename: replace / with _ and spaces with _
...
if [ -z "$REVIEW_JSON" ]; then
 ...
 nvm use "${NODE_VERSION:-system}" > /dev/null 2>&1 || nvm use --lts > /dev/null 2>&1
 echo -e "${GREEN}✅ Using node $(node --version) via nvm${NC}"
 else
 echo -e "${RED}❌ nvm not found at $NVM_DIR${NC}"
...
# Sanitise: only valid GitHub review events allowed
...
REVIEW_BODY=$(jq -n \
 --arg body "$(printf '##Code Review\n\n%s\n\n---\n*Reviewed locally using AI*' "$SUMMARY")" \
 --arg event "$VERDICT" \
...
echo ""
echo -e "${GREEN}🎉 Done! View PR at: https: